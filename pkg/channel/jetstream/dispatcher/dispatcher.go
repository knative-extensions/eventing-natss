...
func (d *Dispatcher) messageReceiver(ctx context.Context, ch eventingchannels.ChannelReference, message binding.Message, transformers []binding.Transformer, _ nethttp.Header) error {
	...
	additionalTransformers := append(transformers, tpTsTransformers...)
	additionalTransformers = append(additionalTransformers, &eventID)

	writer := new(bytes.Buffer)
	if err := cejs.WriteMsg(ctx, message, writer, additionalTransformers...); err != nil {
